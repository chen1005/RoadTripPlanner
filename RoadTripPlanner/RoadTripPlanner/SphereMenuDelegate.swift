//
//  SphereMenuDelegate.swift
//  RoadTripPlanner
//
//  Created by Rick Chen on 15/12/11.
...
class SphereMenu:UIView, UICollisionBehaviorDelegate{
    ...
    func commonSetup()
    {
        ...
        for (var i = 0; i < self.count; i++) {
            ...
            let position = self.layoutToBottomForSphereAtIndex(i) // Zhuo Chen - change launch style
    ...
    func layoutToBottomForSphereAtIndex(index:Int) -> CGPoint
    {
        let startPoint = self.center
        let x = startPoint.x
        let y = startPoint.y + kSphereLength / 1.5 * CGFloat(index + 1)
        let position = CGPointMake(x, y)
        return position
    }
